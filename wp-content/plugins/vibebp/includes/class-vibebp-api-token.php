	        ...
	        try {
	            ...
	            if($expanded_token){
	            	...
	            	if(in_array($expanded_token->vibebp_token_key, array_keys($tokens))){
				        ...
				        //checkout page vibebp login hack when someone logs in at checkout page 
	            		/*if(function_exists('wc_get_page_permalink')){
	            			$checkout_url = wc_get_page_permalink( 'checkout' );
	            			if(!empty($checkout_url)){
	            				$cparams = explode('/', $checkout_url);
	            				if(!empty($cparams)){
	            					$checkout_slug = $cparams[count($cparams)-1];
			            			if(empty($cparams[count($cparams)-1]) && !empty($cparams[count($cparams)-2])){
			            				$checkout_slug=$cparams[count($cparams)-2];
			            			}
			            			if(!empty($headers) && !empty($headers['referer']) && !empty($headers['referer'][0]) && strpos($headers['referer'][0], $checkout_slug) !==false){
				            			$data['redirect_component'] = $headers['referer'][0];
