...
class VibeBP_PostTypes{
	...
	function print_default_script($hook){
		if($hook == 'edit.php' && isset($_GET['post_type']) && in_array($_GET['post_type'],$this->posts)){
	...
	function mark_default(){
	    ...
	    update_post_meta($_POST['id'],'default_'.$post_type,intval($_POST['default']));

	    do_action('update_default_post',$_POST['id'],$_POST['default']);
	    die();
	}
