						...
						if(!empty($commission['set'])){
	                        ...
	                        if( $commission['commission'] ){ 
								...
								if(isset($commission['currency'])){
									...
									update_user_meta($user_id,'last_payout_timestamp',time());
...
//MANISH's CODE --- 
		...
		$lists  = $wpdb->get_results("
			SELECT `user_id`, meta_value as currency FROM {$wpdb->usermeta} 
			WHERE `meta_key` = 'vibe_request_payouts'", ARRAY_A);
...
//MANISH's CODE END ----	
	...
	function lms_commission_settings(){
		...
		$courses = get_posts(apply_filters('vibe_earnings_set_commission_post_ids',[
			'post_type' => ['course'],
			 'posts_per_page'=> -1
		]));

		echo '<form method="POST"><div class="postbox instructor_info">
						<h3><label>'.__('Item Name','wplms').'</label><span>'.__('Instructor','wplms').'</span><span>'.__('PERCENTAGE','wplms').'</span></h3>
						<div class="inside">
							<ul>';
