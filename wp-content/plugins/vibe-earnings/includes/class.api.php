...
class Vibe_Earnings_API extends WP_REST_Controller	{
    ...
    function request_payouts($request){
	    ...
	    $id = bp_activity_add( apply_filters('vibe_earnings_record_activity',
	    	array( 
    			'user_id' => $this->user->id, 
    			'action' => _x('Commission payout requested','Payout request for activity action','vibe-earnings'),
                'content' => sprintf(_x('You requested for commission payout on %s','Payout request for activity action','vibe-earnings'),date($date_format,time())),
                'component' => 'vibe_earnings',
                'type' => 'payout_request',
		...
		return new WP_REST_Response( ['status'=>1,'last_payout'=>date($date_format,time()),'message'=>__('Payout request sent','vibe-earnings')], 200 );
	}
	...
	function get_last_payout_request($request){
		...
		$last_payout_timestamp = get_user_meta($user_id,'last_payout_timestamp',true);
		//Las payout request in 1 year
		if(empty($last_payout_timestamp)){
			$last_payout_timestamp='2014-02-18 16:36:12';}
		else{
			$last_payout_timestamp=date('M j Y g:i A', strtotime($last_payout_timestamp));
		}

		$return = array('status'=>0);
		$results = $wpdb->get_results($wpdb->prepare("SELECT  DATE(activity.date_recorded) as last_time
	                                      FROM {$bp->activity->table_name} as activity
	                                      WHERE     activity.component     = 'vibe_earnings'
	                                      AND     activity.type     = 'payout_request'
	                                      AND     activity.user_id     = {$user_id}
	                                      AND    DATE(activity.date_recorded) > %s
	                                      ORDER BY last_time DESC LIMIT 1",$last_payout_timestamp),ARRAY_A);
		;
		if(!empty($results)) {
	...
	function get_threshold_commission($request){
		...
		$commissions = $wpdb->get_results( "
	                                      ...
	                                      WHERE     activity.component  IN ('course','appointment')
	                                      AND     activity.type   IN ('course_commission','appointment_commission')
	                                      AND     activity.user_id     = {$user_id}
	...
	function get_instructor_commissions_rows($request){
		...
		$date_start = $post['date']['start'];	
		$date_end = $post['date']['end'];
		...
		if(function_exists('wplms_plugin_load_translations') && !empty($post['item']['type']) && $post['item']['type'] == 'course'){
			$course_id =$post['item']['id'];
			if(!empty($course_id)){
        ...
        $results = $wpdb->get_results($wpdb->prepare("
                                      SELECT SQL_CALC_FOUND_ROWS activity.secondary_item_id as order_item_id,activity.item_id as item_id,activity.type as item,meta.meta_value as commission,meta2.meta_value as currency,activity.date_recorded as date
                                      FROM {$bp->activity->table_name} AS activity 
                                      LEFT JOIN {$bp->activity->table_name_meta} as meta ON activity.id = meta.activity_id
                                      LEFT JOIN {$bp->activity->table_name_meta} as meta2 ON activity.id = meta2.activity_id
                                      WHERE     activity.component     IN ('course','appointment')
                                      ...
                                      LIMIT %d, %d",$user_id,'_commission'.$user_id,'_currency'.$user_id,$offset,$per_page),ARRAY_A);
        ...
        if(!empty($data) && isset($data)) {
        	foreach($data as $key =>$value) {
        		...
        		$data[$key]['timestamp']= $t*1000;
        		$data[$key]['date'] = wp_date( get_option( 'date_format' ), $t);
        		$data[$key]['student'] = bp_core_get_user_displayname($user_id);
        		if($value['item'] == 'course_commission'){
        			$data[$key]['type']= get_the_title($value['item_id']);
        		}
        		if($value['item'] == 'appointment_commission'){
        			$data[$key]['type']= sprintf(_x('Appointment Booking [%d]','vibe-earnings'),$value['item_id']);
        		}
        	}
        	...
        	$datas = [
        		'columns'=>[
        			['title'=>_x('Date','table column label ','vibe-earnings'),'field'=>'date'],
					['title'=>_x('Student','table column label','vibe-earnings'),'field'=>'student'],
					['title'=>_x('Type','table column label','vibe-earnings'),'field'=>'type'],
					['title'=>_x('Commission','table column label','vibe-earnings'),'field'=>'commission'],
					['title'=>_x('Currency','table column label','vibe-earnings'),'field'=>'currency'],
        		],
        		'data'=>$data
        	];
        }
		...
		if($count == 0){$status=0;}else{$status=1;}
		$return = array('status'=>$status,'data'=>$datas,'last_row'=>$count);
	...
	function get_instructor_payouts($request){
        ...
        $results = $wpdb->get_results( $wpdb->prepare("
            SELECT SQL_CALC_FOUND_ROWS posts.post_date_gmt as date, meta.meta_value as payout,meta2.meta_value as currency 
            FROM {$wpdb->posts} as posts
            LEFT JOIN {$wpdb->postmeta} as meta on posts.id = meta.post_id
            LEFT JOIN {$wpdb->postmeta} as meta2 on posts.id = meta2.post_id
            WHERE meta.meta_key = %s 
            AND meta2.meta_key = %s 
            AND posts.post_type = 'payments'
            ".$and_where."
            ORDER BY date ASC
            LIMIT %d,%d",'payout_'.$user_id,'currency_'.$user_id,$offset,$per_page),ARRAY_A);
        ...
        if(!empty($data) && isset($data)) {
        	...
        	$datas = [
        		'columns'=>[
        			['title'=>_x('Date','table column label ','vibe-earnings'),'field'=>'date'],
					['title'=>_x('Payout','table column label','vibe-earnings'),'field'=>'payout'],
					['title'=>_x('Currency','table column label','vibe-earnings'),'field'=>'currency'],
        		],
        		'data'=>$data
        	];
